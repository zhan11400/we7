...
class Create_EweiShopV2Page extends MobileLoginPage
{
    ...
    function main()
    {
        ...
        $show_card=true; //是否显示会员卡选项
        if(p('exchange')){//exchange
            $exchangeOrder = trim($_GPC['exchange']);//兑换中心订单
            ...
            if (empty($id)) {
                ...
                if(!empty($quickid)){
                    $sql = 'SELECT c.goodsid,c.total,g.maxbuy,g.type,g.intervalfloor,g.intervalprice,g.issendfree,g.isnodiscount,g.ispresell,g.presellprice as gpprice,o.presellprice,g.preselltimeend,g.presellsendstatrttime,g.presellsendtime,g.presellsendtype'
                        ...
                        . ' g.storeids,g.isverify,g.isforceverifystore,g.deduct,g.manydeduct,g.virtual,o.virtual as optionvirtual,discounts,'
                        ...
                        . ' ,g.isdiscount,g.isdiscount_time,g.isdiscount_discounts,g.cates,g.isfullback, '
...
//                    自定义表单
            ...
            foreach ($goods as $g) {
                ...
                , 'wholesaleprice' => $g['wholesaleprice'], 'goodsalltotal' => $g['goodsalltotal'],'isnodiscount'=>$g['isnodiscount'],'deduct'=>$g['deduct'],'deduct2'=>$g['deduct2'],'ggprice'=>$g['ggprice'],'manydeduct'=>$g['manydeduct']);
                if ($g['seckillinfo'] && $g['seckillinfo']['status'] == 0) {
                    //秒杀不管二次购买
                } else {
...
//                $discountprice=0;
        ...
        if (!empty($return_card_array)) {
            ...
            $deductprice2 = $return_card_array['cardgoodprice'];//重新计算最多抵扣
        }
        if ($is_openmerch == 1) {
            //读取多商户营销设置
            foreach ($merch_array as $key => $value) {
        ...
        foreach ($allgoods as &$g) {
            ...
            if (!empty($_GPC['deduct2']) && !empty($saleset['moneydeduct']) && !empty($g['need_deduct2'])) {
                ...
                $order_need_deduct2=$deductprice2;//订单余额抵扣所需的余额
                $goods_need_deduct2=floor($g['need_deduct2']);//商品余额抵扣所需的余额
...
//                pdo_update('ewei_shop_goods', array('sales' => $vgoods['sales'] + $vgoods['total']), array('id' => $vgoods['goodsid']));
    ...
    function caculatecard($card_id, $dispatch_price,$lotterydiscountprice,$taskcut,$goodsarr, $totalprice, $discountprice, $isdiscountprice,$liveid,$isSubmit = 0)
    {
        global $_W;
        $open_redis = function_exists('redis') && !is_error(redis());
        $openid = $_W['openid'];
...
//满额减
        ...
        if (!empty($saleset)) {
            ...
            if (!empty($saleset['moneydeduct'])) {
                ...
                $deductcredit2 = $member['credit2'];
                if ($deductcredit2 > $totalprice-$isdiscountprice-$discountprice-$deductenough_money+$dispatch_price) {
