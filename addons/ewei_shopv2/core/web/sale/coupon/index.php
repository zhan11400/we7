    ...
    function query() {
        ...
        if(!empty($ds)){
            foreach ($ds as &$d) {
                ...
                if($diy){
                    // uselimit 使用限制  values 价值
                    if($d['coupontype']==0){
                        if($d['enough']>0) {
                            $d['uselimit'] ='满'.((float)$d['enough']).'元可用';
                        }else {
                            $d['uselimit'] ='无门槛使用';
                        }
                    }
                    elseif($d['coupontype']==1){
                        if($d['enough']>0) {
                            $d['uselimit'] ='充值满'.((float)$d['enough']).'元可用';
                        }else {
                            $d['uselimit'] ='充值任意金额';
                        }
                    }
                    ...
                    if($d['backtype']==0){
                        $d['values']='￥'.((float)$d['deduct']);
                    }
                    elseif($d['backtype']==1){
                        $d['values']=((float)$d['discount']).'折 ';
                    }
                    elseif($d['backtype']==2){
                        $values = 0;
                        if (!empty($d['backmoney']) && $d['backmoney'] > 0) {
                            $values = $values + $d['backmoney'];
                        }
                        if (!empty($d['backcredit']) && $d['backcredit'] > 0) {
                            $values = $values + $d['backcredit'];
                        }
                        if (!empty($d['backredpack']) && $d['backredpack'] > 0) {
                    ...
                    elseif($d['backtype']==2){
                        ...
                        }elseif (!empty($d['backcredit']) && $d['backcredit'] > 0) {
                            $d['value_text'] = '积分';
                            $d['value_total'] = $d['backcredit'];
                        }elseif (!empty($d['backredpack']) && $d['backredpack'] > 0) {
                            $d['value_text'] = '0';
                ...
                unset($d['respdesc'],$d['respthumb'],$d['resptitle']);
            }
            unset($d);
        }
