...
//        $sql = "select log.id,m.id as mid, g.realname,m.avatar,m.weixin,log.logno,log.type,log.status,log.rechargetype,log.sendmoney,m.nickname,m.mobile,g.groupname,log.money,log.createtime,l.levelname,log.realmoney,log.deductionmoney,log.charge,log.remark,log.alipay,log.bankname,log.bankcard,log.realname as applyrealname,log.applytype from " . tablename('ewei_shop_member_log') . " log "
        ...
        if ($log['rechargetype'] == 'wechat') {
            ...
            if(!empty($log['apppay'])){
                ...
                $res = m('finance')->AlipayRefund(array(
                    ...
                    'refund_reason'=> "会员充值退款: {$log['money']}元 订单号: " . $log['logno'].'/'.$out_refund_no,
