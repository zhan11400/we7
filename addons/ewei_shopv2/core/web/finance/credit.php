...
//        $params = array(':uniacid' => $_W['uniacid'],':credittype' => $type);
        ...
        if($_GPC['groupid'] || $_GPC['level'] || $_GPC['keyword']){
            ...
            if($type == 'credit1'){
                ...
                $table1 = "select log.id,log.num,log.createtime,log.remark,log.credittype,m.id as mid,m.openid, m.realname,m.nickname,m.avatar, m.mobile, m.weixin,u.username,g.groupname,l.levelname from " . tablename('mc_credits_record') . " log "
                    . " left join " . tablename('users') . " u on  log.operator=u.uid and log.operator<>0 and log.operator<>log.uid"
                    . " left join " . tablename('ewei_shop_member') . " m on m.uid=log.uid"
                    . " left join " . tablename('ewei_shop_member_group') . " g on m.groupid=g.id"
                    . " left join " . tablename('ewei_shop_member_level') . " l on m.level =l.id"
                    . " where 1 {$condition} and log.uid<>0";
                $table2 = "select log.id,log.num,log.createtime,log.remark,log.credittype,m.id as mid,m.openid, m.realname,m.nickname,m.avatar, m.mobile, m.weixin,u.username,g.groupname,l.levelname  from " . tablename('ewei_shop_member_credit_record') . " log "
                    . " left join " . tablename('users') . " u on  log.operator=u.uid and log.operator<>0 and log.operator<>log.uid"
                ...
                $table1 = "select log.id,log.num,log.createtime,log.remark,log.credittype,m.id as mid,m.openid, m.realname,m.nickname,m.avatar, m.mobile, m.weixin,u.username,g.groupname,l.levelname  from " . tablename('mc_credits_record') . " log "
                    ...
                    . " left join " . tablename('ewei_shop_member_level') . " l on m.level =l.id"
                    . " where 1 {$condition} and log.uid<>0";
//                $table2 = "select log.id,log.money,log.createtime,log.remark,'credit2' as credittype,m.id as mid,m.openid, m.realname,m.nickname,m.avatar, m.mobile, m.weixin,log.rechargetype as username,g.groupname,l.levelname from " . tablename('ewei_shop_member_log') . "as log "
//                    . " inner join " . tablename('ewei_shop_member') . " m on m.openid=log.openid"
//                    . " left join " . tablename('ewei_shop_member_group') . " g on m.groupid=g.id"
//                    . " left join " . tablename('ewei_shop_member_level') . " l on m.level =l.id"
//                    . " where m.uid=0 and log.status=1 {$condition1}";
            ...
            $table1 = "select log.id,log.num,log.createtime,log.remark,log.credittype,u.username,log.uid,'xxx' as openid from " . tablename('mc_credits_record') . " log "
                . " left join " . tablename('users') . " u on  log.operator=u.uid and log.operator<>0 and log.operator<>log.uid"
                . " where 1 {$condition} and log.uid<>0";
            $table2 = "select log.id,log.num,log.createtime,log.remark,log.credittype,u.username,'0' as uid,log.openid  from " . tablename('ewei_shop_member_credit_record') . " log "
                . " left join " . tablename('users') . " u on  log.operator=u.uid and log.operator<>0 and log.operator<>log.uid"
                . " where 1 {$condition} and log.uid=0";
            //$sql = "select * from (".$table1." UNION ALL ".$table2.") as main order by createtime desc";
            //正常显示列表的时候 , 使用咱们自己的表(ewei_shop_member_credit_record)记录
            $sql = "select log.id,log.num,log.createtime,log.remark,log.credittype,u.username,'0' as uid,log.openid  from " . tablename('ewei_shop_member_credit_record') . " log "
                . " left join " . tablename('users') . " u on  log.operator=u.uid and log.operator<>0 and log.operator<>log.uid"
                . " where 1 {$condition} order by log.createtime desc";
        }
        ...
        if ($_GPC['export'] == 1) {
            ...
            m('excel')->export($list, array(
                "title" => ($type == 'credit1' ? "会员积分明细-" : "会员余额明细") . date('Y-m-d-H-i', time()),
                "columns" => $columns
            ));
        }
        ...
        if($type == 'credit1'){
            ...
            if(empty($search_flag)){
                $total_sql = "select count(*) as ccc  from " . tablename('ewei_shop_member_credit_record') . " log "
                    . " left join " . tablename('users') . " u on  log.operator=u.uid and log.operator<>0 and log.operator<>log.uid"
                    . " where 1 {$condition}";
