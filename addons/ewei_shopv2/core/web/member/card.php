...
class Card_EweiShopV2Page extends WebPage {
    ...
    function activationset()
    {
        ...
        if ($_W['ispost']) {
            ...
            $data = array(
                "centerget" => intval($_GPC['centerget']),
                "openactive" => intval($_GPC['openactive']),
                "realname" => intval($_GPC['realname']),
                "mobile" => intval($_GPC['mobile']),
                "birthday" => intval($_GPC['birthday']),
                "idnumber" => intval($_GPC['idnumber']),
                "sms_active" => intval($_GPC['sms_active']),
                "sms_id" =>intval($_GPC['sms_id']), //开启短息验证
                "credit1" => intval($_GPC['credit1']), //积分
                "credit2" => floatval($_GPC['credit2']),  //余额
                "couponid" => intval($_GPC['couponid']),  //余额
                "levelid" => intval($_GPC['levelid'])  //余额
            );
