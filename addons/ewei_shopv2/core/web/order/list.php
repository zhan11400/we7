        ...
        if ($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0 and o.istrade=0 ' || !empty($sqlcondition)){
            ...
            if($searchfield == 'member') {
                ...
                $sql = "select o.* , a.realname as arealname,a.mobile as amobile,a.province as aprovince ,a.city as acity , a.area as aarea, a.street as astreet,a.address as aaddress,
                      ...
                      r.rtype,r.status as rstatus,o.sendtype,o.city_express_state from " . tablename('ewei_shop_order') . " o"
            ...
            foreach ($list as &$value) {
                ...
                if ($s == -1) {
                    ...
                    if(!empty($value['refundid']) && !empty($_GPC['export'])){
                        $applyprice = pdo_fetchcolumn('select applyprice from ' . tablename('ewei_shop_order_refund') . ' where id=:id and uniacid=:uniacid limit 1', array(':id'=>$value['refundid'],':uniacid' => $_W['uniacid']));
                    }
                }
        ...
        if ($_GPC['export'] == 1) {
            ...
            foreach ($list as &$r) {
                ...
                foreach ($ogoods as $k => $g) {
                    ...
                    $r['goods_costprice'] = $g['costprice']==0?'':$g['costprice'];//成本价为零时不输出
                    $r['goods_diyformdata'] = $g['goods_diyformdata'];

                    foreach ($diy_title_data as $key => $value) {
                        $field = 'goods_' . $key;
                        $diy_value = $g[$field];
                        if(strlen($diy_value)>15 && is_numeric($diy_value)){
                            $diy_value = '`'.$diy_value;
                        }
                        $r[$field] = $diy_value;
                    }
        ...
        if ($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0' || !empty($sqlcondition)){
            if ($searchfield == 'member') {
                ...
                $orderPrice = pdo_fetch('SELECT count(1) as count,sum(o.price) as sumprice FROM '.tablename('ewei_shop_order').
                    ' o left join '.tablename('ewei_shop_member_address').' a on o.addressid = a.id WHERE o.uniacid = '.$_W['uniacid'].'  AND o.deleted=0 AND o.isparent=0 '.$priceCondition.$statuscondition);
                $totalmoney = $orderPrice['sumprice'];
                $total = $orderPrice['count'];
                ...
                if($status === '' && empty($_GPC['keyword']) && empty($_GPC['time'])){
                    ...
                    if(!is_error($redis)){
                        if ($redis->get($redis_key_total) != false && $redis->get($redis_key_totalmoney) != false) {  //判断key值所对应的值是否存在
                            $total =  $redis->get($redis_key_total);
                            $totalmoney =  $redis->get($redis_key_totalmoney);
                        }else {
                            $t = pdo_fetch(
                                'SELECT COUNT(DISTINCT(o.id)) as count, ifnull(sum(o.price),0) as sumprice   FROM ' . tablename('ewei_shop_order') . " o "
                                . " left join " . tablename('ewei_shop_order_refund') . " r on r.id =o.refundid "
                                . " left join " . tablename('ewei_shop_saler') . " s on s.openid = o.verifyopenid and s.uniacid=o.uniacid"
                                . " left join " . tablename('ewei_shop_member') . " m on m.openid = s.openid and m.uniacid=s.uniacid"
                                . " $sqlcondition WHERE $condition $statuscondition", $paras);
                            if(!empty($heads)){
                            $redis->set($redis_key_total, $t['count'], array('nx', 'ex' => '300'));  //设置缓存  过期时间默认为5分钟
                            $redis->set($redis_key_totalmoney, $t['sumprice'], array('nx', 'ex' => '300'));  //设置缓存  过期时间默认为5分钟
                              }
                            $total =  $t['count'];
            ...
            $t = pdo_fetch(
                'SELECT COUNT(*) as count, ifnull(sum(price),0) as sumprice   FROM ' . tablename('ewei_shop_order') . " WHERE uniacid = :uniacid and ismr=0 and deleted=0 and isparent=0 {$status_condition}", $paras);
            $total = $t['count'];
