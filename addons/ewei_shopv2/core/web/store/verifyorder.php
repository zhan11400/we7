    ...
    protected  function orderData(){
        ...
        if (!empty($searchtime) && is_array($_GPC['time']) && in_array($searchtime, array('create', 'pay', 'send', 'finish'))) {
            ...
            if ($searchtime == 'finish') {
                $condition .= " And vgl.verifydate >= :starttime And vgl.verifydate <= :endtime ";
            } else {
...
//                $condition .= " AND ( o.storeid =0  and   o.isnewstore=0 and o.isverify=1 )";
...
//        if ($condition != ' o.uniacid = :uniacid and o.ismr=0 and o.deleted=0 and o.isparent=0 and o.istrade=0 ' || !empty($sqlcondition)){
        $sql = "select o.* , a.realname as arealname,a.mobile as amobile,a.province as aprovince ,a.city as acity , a.area as aarea, a.street as astreet,a.address as aaddress,
                  ...
                  r.rtype,r.status as rstatus,o.sendtype,store.storename{$field} from " . tablename('ewei_shop_order') . " o"
            ...
            .  " left join " . tablename('ewei_shop_store') . " store on store.id = o.verifystoreid"
            .$leftsql
            . " $sqlcondition where $condition AND o.status =3 GROUP BY o.id ORDER BY o.createtime DESC  ";
...
//                        $value['addressdata']['realname'] = $value['mobile'] = $carrier['carrier_realname'];
//                        $value['addressdata']['storename'] = $value['mobile'] = $carrier['storename'];
                    }
                ...
                $order_goods = pdo_fetchall('select g.id,g.title,g.thumb,g.goodssn,og.goodssn as option_goodssn, g.productsn,og.productsn as option_productsn, og.total,
                    ...
                    og.diyformfields,op.specs,g.merchid,og.seckill,og.seckill_taskid,og.seckill_roomid,g.ispresell from ' . tablename('ewei_shop_order_goods') . ' og '
...
//            }
        ...
        $t = pdo_fetch(
            ...
            .  " left join " . tablename('ewei_shop_store') . " store on store.id = o.verifystoreid"
            .$leftsql
            . " $sqlcondition WHERE $condition AND o.status =3", $paras);
