...
class Notice_EweiShopV2Model {
    ...
    public function sendOrderMessage($orderid = '0', $delRefund = false,$raid = null) {
        ...
        $order_goods = pdo_fetchall("select g.id,g.title,og.realprice,og.total,og.price,og.optionname as optiontitle,g.noticeopenid,g.noticetype,og.sendtype,og.expresscom,og.expresssn,og.sendtime from " . tablename('ewei_shop_order_goods') . " og "
            . " left join " . tablename('ewei_shop_goods') . " g on g.id=og.goodsid "
            . " where $scondition and og.uniacid=:uniacid ", $param);
        $goods = '';
        $goodsname = '';
        ...
        $couponstr = "";
        if($order['couponid']>0 && $order['couponprice']){
        ...
        //如果是虚拟卡密订单并且 虚拟卡密内容 为空时 则在redis里面取值
                ...
                if ($refund['status'] == 0) {
                    ...
                    if(!empty($is_send)){
                        ...
                        $msg = array(
                            ...
                            'keyword2' => array('title' => '业务内容', 'value' => '维权申请通知', "color" => "#000000"),
                            'keyword3' => array('title' => '处理结果', 'value' => $r_type[$refund['rtype']].'申请', "color" => "#4b9528"),
                            'keyword4' => array('title' => '操作时间', 'value' => date('Y-m-d H:i:s'), "color" => "#000000"),
                    ...
                    if ($refund['rtype'] == 2||$refund['rtype'] == 1) {
                        ...
                        $remark2 = "请将快递发送到以下地址，并随包裹填写您的订单编号以及联系方式，我们将尽快为您处理\n\n邮寄地址："
                            .$salerefund["province"] . $salerefund["city"] . $salerefund["area"] . " " . $salerefund["address"].
                            "\n联系电话：".$salerefund['mobile']."\n收货人：".$salerefund['name']."\n\n感谢您关注，如有疑问请联系在线客服或点击查看详情";

                        $msg = array(
                            'first' => array('value' => "您好，您的退换货申请已经通过，请您及时发送快递。\n", "color" => "#ff0000"),
                            'keyword2' => array('title' => '业务类型', 'value' => '退换货申请', "color" => "#000000"),
                            'keyword3' => array('title' => '业务内容', 'value' => '会员通知', "color" => "#4b9528"),
                            'keyword1' => array('title' => '处理结果', 'value' => '退换货通过', "color" => "#000000"),
                    ...
                    $msg = array(
                        ...
                        'keyword2' => array('title' => '业务类型', 'value' => '退换货申请', "color" => "#000000"),
                        'keyword3' => array('title' => '业务内容', 'value' => '会员通知', "color" => "#ff0000"),
                        'keyword1' => array('title' => '处理结果', 'value' => '驳回通知', "color" => "#000000"),
                        'keyword4' => array('title' => '操作时间', 'value' => date('Y-m-d H:i:s', time()), "color" => "#000000"),
                        'remark' => array('value' => $remark2, "color" => "#000000")
                    );
...
//						));
                ...
                if (!empty($og['noticeopenid']) && !empty($og['noticetype'])) {
                    ...
                    if ($og['noticetype'] == '1' || (is_array($noticetype) && in_array('1', $noticetype) )) {
                        ...
                        $msg = array(
                            'first' => array('value' => "您有新的货到付款订单于" . date('Y-m-d H:i', $order['createtime']) . "已下单！！\n请登录后台查看详情并及时安排发货。\n", "color" => "#ff0000"),
                            'keyword1' => array('title' => '业务类型', 'value' => '会员通知', "color" => "#000000"),
                            'keyword2' => array('title' => '业务内容', 'value' => '已付款', "color" => "#000000"),
                            'keyword3' => array('title' => '处理结果', 'value' => '商品付款通知', "color" => "#000000"),
                            'keyword4' => array('title' => '操作时间', 'value' => date('Y-m-d H:i:s', time()), "color" => "#000000"),
                            'remark' => array('value' => $remark, "color" => "#000000")
                        );
        ...
        /*else if ($order['status'] == 0) {
            ...
            $is_send = 0;
            if(empty($is_merch)) {
                if(empty($usernotice['saler_submit'])){
                    $is_send = 1;
                }
            } else {
                if (!empty($merch_tm) && empty($merch_tm['saler_submit_close_advanced'])) {
                    $is_send = 1;
                    $tm['openid3'] = $merch_tm['openid3'];
                }
            }
            ...
            // 短信通知
            if (!empty($tm['mobile3']) && empty($tm['saler_finish_close_sms'])) {
        ...
        /*else  if ($order['status'] == 1 &&!empty($order['isnewstore'])) {

        }*/
        else  if (($order['status'] == 1 && empty($order['sendtype'])) || ($order['status'] == 3 && $order['isvirtualsend'] ==1)) {
...
//                    核销单
        ...
        } else if ($order['status'] == 2||($order['status'] == 1 &&!empty($order['sendtype']))) {
            ...
            if ($pv && !empty($order['virtual'])) {
                ...
                if(empty($usernotice['virtualsend'])){
                    ...
                    $msg = array(
                        ...
                        //'remark' => array('title' => '', 'value' => '点击查看详情!', "color" => "#000000")
                    );
                    //   注释掉发送的代码改为sql存储4094
                    $this->sendNotice(array(
                        "openid" => $openid,
...
//                        'uniacid'=>$_W['uniacid'],
//                    );
//                    pdo_insert('ewei_shop_virtual_send_log', $insert);
                ...
                if(!empty($is_send)){
                    $msg = array(
                        'first' => array('value' => $first, "color" => "#4b9528"),
                        'keyword1' => array('title' => '业务类型', 'value' => '会员通知', "color" => "#000000"),
                        'keyword2' => array('title' => '业务内容', 'value' => '虚拟物品及卡密自动发货', "color" => "#000000"),
                        'keyword3' => array('title' => '处理结果', 'value' => '订单收货通知', "color" => "#000000"),
                        'keyword4' => array('title' => '操作时间', 'value' => date('Y-m-d H:i:s', time()), "color" => "#000000"),
                        'remark' => array('title' => '', 'value' => $remark, "color" => "#000000")
                    );
...
//                    pdo_insert('ewei_shop_virtual_send_log', $insert);
                ...
                if(!empty($is_send)){
                    $msg = array(
                        'first' => array('value' => $first, "color" => "#4b9528"),
                        'keyword1' => array('title' => '业务类型', 'value' => '会员通知', "color" => "#000000"),
                        'keyword2' => array('title' => '业务内容', 'value' => '商品自动发货', "color" => "#000000"),
                        'keyword3' => array('title' => '处理结果', 'value' => '订单收货通知', "color" => "#000000"),
                        'keyword4' => array('title' => '操作时间', 'value' => date('Y-m-d H:i:s', time()), "color" => "#000000"),
                        'remark' => array('title' => '', 'value' => $remark, "color" => "#000000")
                    );
...
//							));
                ...
                if(!empty($is_send)){
                    ...
                    $msg = array(
                        'first' => array('value' => $first, "color" => "#4b9528"),
                        'keyword1' => array('title' => '业务类型', 'value' => '会员通知', "color" => "#000000"),
                        'keyword2' => array('title' => '业务内容', 'value' => '商品确认收货', "color" => "#000000"),
                        'keyword3' => array('title' => '处理结果', 'value' => '订单收货通知', "color" => "#000000"),
                        'keyword4' => array('title' => '操作时间', 'value' => date('Y-m-d H:i:s', time()), "color" => "#000000"),
                        'remark' => array('title' => '', 'value' => $remark, "color" => "#000000")
                    );
                ...
                foreach ($order_goods as $og) {
                    ...
                    if ($og['noticetype'] == '2' || (is_array($noticetype) && in_array('2', $noticetype) )) {
                        ...
                        $msg = array(
                            'first' => array('value' => $first, "color" => "#4b9528"),
                            'keyword1' => array('title' => '业务类型', 'value' => '会员通知', "color" => "#000000"),
                            'keyword2' => array('title' => '业务内容', 'value' => '订单收货通知', "color" => "#000000"),
                            'keyword3' => array('title' => '处理结果', 'value' => '虚拟物品及卡密自动发货', "color" => "#000000"),
                            'keyword4' => array('title' => '操作时间', 'value' => date('Y-m-d H:i:s', time()), "color" => "#000000"),
                            'remark' => array('title' => '', 'value' => $remark, "color" => "#000000")
                        );
...
//{{first.DATA}}业务类型：{{keyword1.DATA}}处理状态：{{keyword2.DATA}}处理内容：{{keyword3.DATA}}{{remark.DATA}}
        $oldlevelname = empty($oldlevel['levelname']) ? '普通会员' : $oldlevel['levelname'];
        $message = array(
            'first' => array('value' => "亲爱的" . $member['nickname'] . "，恭喜您成功升级！\n", "color" => "#ff0000"),
            'keyword1' => array('title' => '业务类型', 'value' => '会员通知', "color" => "#000000"),
            'keyword2' => array('title' => '业务内容', 'value' => '您会员等级从 ' . $oldlevelname . ' 升级为 ' . $level['levelname'] . ', 特此通知!', "color" => "#000000"),
            'keyword3' => array('title' => '处理结果', 'value' => '会员升级', "color" => "#000000"),
            'keyword4' => array('title' => '操作时间', 'value' => date('Y-m-d H:i:s', time()), "color" => "#000000"),
    ...
    public function sendMemberPointChange($openid,$pointchange = 0,$changetype=0,$from=0) {
        ...
        {
            $pointcolor='#0a4b9c';
            $pointtext = '增加'.(float)$pointchange.$credittext;
        ...
        else if($changetype==1)
        {
            ...
            $pointtext = '减少'.(float)$pointchange.$credittext;
        }
        ...
        $datas = array(
            ...
            array("name" => "积分余额", "value" => (float)$member['credit1'].$credittext)
        );
        ...
        $message = array(
            ...
            'keyword4' => array('title' => '当前积分', 'value' =>  (float)$member['credit1'].$credittext, "color" => "#ff0000"),
    ...
    public function sendStockWarnMessage($goodsid,$optionid)
    {
        ...
        if(!empty($is_send)){
            ...
            $msg = array(
                ...
                'keyword1' => array('title' => '业务类型', 'value' => '会员通知', "color" => "#000000"),
                'keyword2' => array('title' => '业务内容', 'value' => '请及时补货', "color" => "#000000"),
                'keyword3' => array('title' => '处理结果', 'value' => '商品库存不足', "color" => "#4b9528"),
                'keyword4' => array('title' => '操作时间', 'value' => date('Y-m-d H:i:s', time()), "color" => "#000000"),
...
//					));
    ...
    public function sendNotice(array $params) {
        ...
        if (!empty($templateid)) {
            ...
            /*$member = m('member')->getMember($touser);
            ...
            $insert = array(
                ...
                'taskid'=>-1,
                'status'=>-1,
                'log'=>"templateid: {$templatetype['templateid']} -- ".$ret['message']
            );
            pdo_insert('ewei_message_mass_sign', $insert);*/
        }
        return $ret;
    }
    ...
    //订单状态更新 (修改收货地址)(修改价格)-orderstatus
    ...
    //根据消息标签与uid 申请微信模板
    function settemplateidbyback($tag,$uniacid=0)
    {
        global $_W, $_GPC;
        ...
        foreach($result['template_list']  as $key => $value)
        {
            ...
            if(str_replace("\n","",$value['content'])==$content)
            {
                ...
                $defaulttemp = pdo_fetch('select 1  from ' . tablename('ewei_shop_member_message_template_default') . ' where typecode=:typecode and uniacid=:uniacid  limit 1',
                    array(':typecode' => $tag,':uniacid' => $uniacid));
                if(empty($defaulttemp))
                {
                    pdo_insert('ewei_shop_member_message_template_default',
                        array('typecode' =>$tag,'uniacid' => $uniacid,'templateid'=>$value['template_id']));
                }
                else
                {
