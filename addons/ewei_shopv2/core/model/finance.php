...
class Finance_EweiShopV2Model
{
    ...
    public function payRedPack($openid,$money,$logno,$data,$desc='红包发送',$config)
    {
        ...
        //已经发送的金额
        $sendmoney = (float)$data['sendmoney'];
        //还需要发送的金额
        ...
        $params = array(
            'openid'=>$openid,
            'tid'=>'',
            'send_name'=> mb_substr($shopset['name'],0,10,'UTF-8'),
            'money'=>'',
            'wishing'=>'恭喜发财,大吉大利!',
            'act_name'=>$_W['shopset']['shop']['name'].'活动',
            'remark'=>$desc
        );

        if ($realsendmoney > $redpack){
...
//        }
        ...
        if ($payment['is_new'] == 0 && !$app) {
            ...
            if (!empty($pay['weixin_sub'])) {
                $wechat = array(
                    'appid' => $payment['appid_sub'],
                    'mchid' => $payment['mchid_sub'],
                    'sub_appid' => !empty($payment['sub_appid_sub']) ? $payment['sub_appid_sub'] : '',
                    'sub_mch_id' => $payment['sub_mchid_sub'],
                    'apikey' => $payment['apikey_sub']
                );
        ...
        if ($payment['is_new'] == 1 && !$app){
            ...
            $sub_wechat = array(
                ...
                'sub_appid' => !empty($payment['sub_appid']) ? $payment['sub_appid'] : '',
        ...
        if($app){
            ...
            $wechat = array(
                'appid' => $payment['app_wechat']['appid'],
                'mchid' => $payment['app_wechat']['merchid'],
                'apikey' => $payment['app_wechat']['apikey']
            );
            $row = array('key'=>$payment['app_wechat']['appid'], 'secret'=>$payment['app_wechat']['appsecret']);
            $certs = array(
                'cert' => $payment['app_wechat']['cert'],
                'key' => $payment['app_wechat']['key'],
                'root' =>$payment['app_wechat']['root']
            );
        }
    ...
    public function AlipayRefund($params,$batch_no,$config)
    {
        ...
        $parameter = array(
            ...
            "notify_url"	=> isset($params['notify_url']) ? $params['notify_url'] : $_W['siteroot'] . "addons/ewei_shopv2/payment/alipay/notify.php",
    ...
    public function isWeixinPay($out_trade_no,$money=0, $app=false) {
        ...
        if($app){
            $wechat = array(
                'version'=>1,
                'apikey'=>$payment['app_wechat']['apikey'],
                'signkey'=>$payment['app_wechat']['apikey'],
                'appid'=>$payment['app_wechat']['appid'],
                'mchid'=>$payment['app_wechat']['merchid']
            );
        }
    ...
    function isAlipayNotify($gpc)
    {
        ...
        if($gpc['sign_type']=="RSA"){
            ...
            $data_gpc = array(
                'total_amount'=>$gpc['total_amount'],
                'timestamp'=>$gpc['timestamp'],
                'sign'=>$gpc['sign'],
                'trade_no'=>$gpc['trade_no'],
                'sign_type'=>$gpc['sign_type'],
                'auth_app_id'=>$gpc['auth_app_id'],
                'charset'=>$gpc['charset'],
            ...
            //unset($gpc['i'],$gpc['c'],$gpc['m'],$gpc['do'],$gpc['r'],$gpc['openid'],$gpc['fromwechat']);
            $verify = $this->RSAVerify($data_gpc, $public_key, true);
            return $verify;
        }else if($gpc['sign_type']=="RSA2"){
            $sec_yuan = m('common')->getSec();
            $sec =iunserializer($sec_yuan['sec']);
            $public_key = $sec['alipay_pay']['public_key'];
            if(empty($public_key)){
