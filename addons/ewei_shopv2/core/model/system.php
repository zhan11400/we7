    ...
    public function getMenu($full = false)
    {
        ...
        if(!empty($allmenus)){
            ...
            foreach ($allmenus as $key=>$val){
                /*
                if($key=='diypage'){
                    if(empty($_W['shopset']['diypage']['setmenu'])){
                        continue;
                    }
                }*/
...
//                    ),
        ...
        //todo 2020-11-03
        unset($shopmenu['sale'],$shopmenu['app'],$shopmenu['plugins'],$shopmenu['store']);

        unset($shopmenu['merch']['items'][4]);
        unset($shopmenu['sysset']['items'][0]['items'][4]);
        unset($shopmenu['sysset']['items'][3],$shopmenu['sysset']['items'][4]);
        return $shopmenu;
    }
        ...
        if($this->merch){
            ...
            if($routes[0]=='system'){
                ...
                if($_W['isfounder'] && $_W['role'] !='vice_founder'){
                   ...
                   $return_arr['menu_items'][] = array(
                       'text'=>'应用中心',
                       'href'=>webUrl('system/plugin/apps'),
                       'blank'=>true
                   );
                   ...
                   $return_arr['menu_items'][] = array(
                       'text'=>'系统更新',
                       'href'=>webUrl('system/auth/upgrade')
                   );*/
                }
                $return_arr['menu_items'][] = array(
                    'text'=>'修改密码',
               ...
               /* $return_arr['menu_items'][] = array(
                    'text'=>'返回系统',
                    'href'=>'./index.php?c=account&a=display&',
                    'blank'=>true
                );*/
            }
            ...
            $arr['notice'] = pdo_fetchall("SELECT * FROM " . tablename('ewei_shop_system_copyright_notice') . " ORDER BY displayorder DESC,createtime DESC LIMIT 5" );
              if(p('commission')) {
                  if ($this->cv('commission.apply.view1')) {
                      $arr['commission1'] = pdo_fetchcolumn("select count(1) from" . tablename('ewei_shop_commission_apply') . " a "
                  ...
                  if ($this->cv('commission.apply.view2')) {
                      $arr['commission2'] = pdo_fetchcolumn("select count(1) from" . tablename('ewei_shop_commission_apply') . " a "
                          . " left join " . tablename('ewei_shop_member') . " m on m.uid = a.mid"
                          . " left join " . tablename('ewei_shop_commission_level') . " l on l.id = m.agentlevel"
    ...
    protected function getOrderTotal($status = 0)
    {
        ...
        if($status==1){
            ...
            $condition = 'uniacid = :uniacid and isparent=0 and ismr=0 and (refundstate>0 and refundid<>0 or (refundtime=0 and refundstate=3)) and deleted=0';
            $params = array(':uniacid' => $_W['uniacid']);
            if($this->merch){
                $condition .= ' and merchid=:merchid';
                $params[':merchid'] = intval($_W['merchid']);
